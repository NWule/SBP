...
[
  {
    $match: {
      InstructionCount: { $gt: 0 },
      AggregatedRating: { $exists: true }
    }
  },
  {
    $bucket: {
      groupBy: "$InstructionCount",
      boundaries: [1, 5, 10, 15, 20, 30],
      default: "Preko 30 koraka",
      output: {
        UkupnoRecepata: { $sum: 1 },
        ProsecnaOcena: { $avg: "$AggregatedRating" },
        ProsecanBrojRecenzija: { $avg: "$ReviewCount" }
      }
    }
  }
]
...
[ 
  { 
    $match: { 
      $text: { $search: "pizza burger nachos mac cheese fries cake" }, 
      Calories: { $lte: 500 }, 
...
[
  ...
  },
  {
    $project: {
      ...
      PraznicniUdeo: { $divide: ["$HolidayReviewCount", "$ReviewCount"] }
    }
  },
